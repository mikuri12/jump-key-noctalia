import { r as createTemplate } from "./core-C0HOUBwg.js";
import { p as regexEscape, t as addOverlay, x as updateNode } from "./utils-Cmdnutew.js";
//#region src/extensions/search/search.ts
var searchTemplate = /* @__PURE__ */ createTemplate("<div style=\"color:#0000;contain:strict;padding:0 var(--_pse) 0 var(--padding-left)\" aria-hidden=true> ");
var matchTemplate = /* @__PURE__ */ createTemplate("<span> ");
var testBoundary = (str, position, pattern = /[_\p{N}\p{L}]{2}/u) => {
	if (!position) return false;
	return pattern.test(str.slice(position - (str.codePointAt(position - 2) > 65535 ? 2 : 1), position + (str.codePointAt(position) > 65535 ? 2 : 1)));
};
		...
		search(str, caseSensitive, wholeWord, useRegExp, selection, filter, pattern) {
			...
			try {
				...
				while (match = regex.exec(searchStr)) {
					...
					if (wholeWord && (testBoundary(value, index, pattern) || testBoundary(value, index + l, pattern))) continue;
...
//#endregion
export { matchTemplate as n, searchTemplate as r, createSearchAPI as t };

//# sourceMappingURL=search-CvZenEEf.js.map