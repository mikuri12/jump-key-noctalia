...
var atRule = /@([\w-]*)(?!\w|-)(?:[^;{"']|"(?:\\[^]|[^\\\n"])*"|'(?:\\[^]|[^\\\n'])*')*$/;
var tagNames = Object.keys(htmlTags).map((tag) => ({
	label: tag,
	icon: "keyword"
}));
var createPropCompletion = (prop, icon) => ({
	label: prop,
	icon,
	insert: prop + ": ;",
...
var getCSSProperties = () => {
	if (!properties) {
		...
		for (let key in style) if (typeof style[key] == "string" && !/-|^moz|^webkit/i.test(key)) {
...
*/
var cssCompletion = (classes, variables) => {
	return (context, editor) => {
		...
		let options;
		let currentStatement = before.slice(Math.max(...[
			"{",
			"}",
			";"
		].map((c) => before.lastIndexOf(c))) + 1).trimStart();
		...
		let setPropCompletion = () => {
			options = Array.from(findWords(context, editor, (type) => type == "variable", /.+/g, variables, true), (name) => inPropValue ? {
				label: name,
		...
		if (getClosestToken(editor, ".tag", 0, 0, pos)) {
			...
			if (atRuleMatch) {
				...
				for (; bracket = brackets[i]; i++) if (bracket[4] == "{" && bracket[1] < pos && brackets[pairs[i]]?.[2] > pos && !hasStyleRules.includes(before.slice(0, bracket[1]).match(atRule)?.[1])) {
					inSelector = "&+>~:.#[".includes(currentStatement[0]);
					break;
				}
				if (inSelector) {
					if (!/\[[^\]]*$/.test(currentStatement)) {
						if (charBefore == ":") if (before[from - 2] == ":") {
							from -= 2;
							...
							options = Array.from(findWords(context, editor, (type) => type == "selector" || type == "class", /.+/g, classes, true), (name) => ({
								label: name,
								icon: "keyword"
							}));
							from--;
				...
				} else if (charBefore != "#") setPropCompletion();
			}
		}
		if (options && (from < pos && /\D/.test(before[from]) || context.explicit)) return {
			from,
			options
		};
	};
...
var properties;
//#endregion
export { atRules, cssCompletion, cssValues, getCSSProperties, pseudoClasses, pseudoElements };
