import { a as languages, c as tokenize, u as withoutTokenizer } from "../../core-DEy9UQvI.js";
import { t as boolean } from "../../patterns-DSInPV_c.js";
//#region src/prism/languages/graphql.js
languages.graphql = {
	"comment": /#.*/,
	"description": {
		pattern: /(?:"""(?:[^"]|"(?!""))*"""|"(?:\\.|[^\\\n"])*")(?=\s*[a-z_])/gi,
	...
	[tokenize](code, grammar) {
		...
		*/
		var isNotTokenType = (types) => {
			for (var i = 0; i < types.length; i++) if (currentIndex + i == l || validTokens[currentIndex + i].type != types[i]) return true;
		};
		/**
		...
		var findClosingBracket = (open, close) => {
			...
			for (var i = currentIndex; i < l; i++) {
				...
				if (token.type == "punctuation") {
					if (open == content) stackHeight++;
					else if (close == content && !--stackHeight) return i;
				}
			}
		};
				...
				if (!isNotTokenType(["punctuation", "property-query"]) && validTokens[currentIndex].content == "{") {
					...
					if (inputVariables[0]) {
						...
						if (mutationEnd) for (var i = currentIndex; i < mutationEnd; i++) {
							...
							if (varToken.type == "variable" && inputVariables.indexOf(varToken.content) >= 0) varToken.alias = "variable-input";
						}
					}
				}
			}
