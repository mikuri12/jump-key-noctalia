...
//#region src/extensions/search/selection.ts
/**
...
var highlightSelectionMatches = (caseSensitive, minLength = 1, maxLength = 200) => {
	const self = (editor) => {
		...
		editor.on("selectionChange", ([start, end], value) => {
			...
			value = value.trim();
			let l = value.length;
			searchAPI.search(minLength > l || l > maxLength ? "" : value, caseSensitive, false, false, void 0, (mStart, mEnd) => mStart > start || mEnd <= start);
		});
	};
	return self;
};
...
* ```
*/
var highlightCurrentWord = (filter, includeHyphens) => {
	const self = (editor) => {
		...
		editor.on("selectionChange", ([start, end], value) => {
			if (start < end || !editor.focused || noHighlight) searchAPI.search("");
				...
				searchAPI.search(/^-*(\d|$)/.test(word) || filter && !filter(index, index + word.length) ? "" : word, true, true, false, void 0, filter, RegExp(group + "{2}", "u"));
			}
			noHighlight = false;
		});
	};
...
//#endregion
export { highlightSelectionMatches as n, highlightCurrentWord as t };

//# sourceMappingURL=selection-ClP9zgI_.js.map